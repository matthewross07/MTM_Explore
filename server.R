...
id1<- reactive({validate(
  need(input$MapMine_shape_click != "",""))
  (input$MapMine_shape_click)
})
  ...
  observe({
    ...
    if(id()$group == 'Watersheds') {
      ...
      output$secondbar <- renderPlot({
        dat <-   hucMine$SUM[which(hucMine$HUC12 == as.character(id()$id))]*100
        barplot(height=6.4,horiz=T,xlim=c(0,7))
        par(new=T)
        pct <- round(dat/(6.4*1000*1000*10),1)
        barplot(height=dat/(1000*1000*1000),xlim=c(0,7),xaxt='n',col='darkred',horiz=T,
                main='Contribution of Watershed to Total Spoil Volume',
                xlab=expression(paste('Volume (',km^3,')'),sep=''))     
        text(x=3.5,y=.5,labels=paste('This watershed contributes ',pct,'% of spoil',sep=''))
      })
      output$kWh <- renderText({
        ...
        output$barplot<- renderPlot({
          ...
          barplot(height= dat/areas$val, horiz=T, log= "x",xlab='Burial depth (m)',
                  las=1, col= "darkgrey",xlim=c(0.1,1000000),
                  main= bquote(paste('Depth of ',.(round(dat/(1000*1000*1000),3)),km^3," of Spoil, if Spread Over Various Areas")))
          text(((dat/areas$val)/4)+3,y= (1:nrow(areas))*1.2, labels=(areas$names), pos=1, font= 2)
        })
      output$secondbar <- renderPlot({
        cols <- rep('gray',times=nrow(CountyCoal))
        cols[CountyCoal$NAME == id()$id] <- 'darkred'
        barplot(height = (CountyCoal$SUM*100)/CountyCoal$coal_m3, names.arg= CountyCoal$NAME, 
                main= "Spoil Deposited into Headwater Valleys per cubic meters of Coal", 
                col= cols, ylab= "cubic metres", xlab="County")
      })
      txt <- CountyCoal$coal_kg[which(CountyCoal$NAME == as.character(id()$id))]
      output$kWh <- renderText({
        paste(txt/8.141, "kilowatt hours of coal energy produced in", as.character(id()$id),"County." )
      })
    }
  }) 
...
fName<- reactive({ff[grep(id()$id,ff)]})
...
observe({
  ...
  if(input$Diff == T){
    path <- normalizePath(file.path('www',
                                    paste("/RastImg/Diff/",fName(),sep='')))
    list(src=path,
         alt=fName())
  }else{
  if(input$ES == 1){ # If slope is called
    if(input$PrePost == T){
      path <- normalizePath(file.path('www',
                            paste("/RastImg/OldS/",fName(),sep='')))
      list(src=path,
           alt=fName())
    }else{ # if New slope is called. 
      path <- normalizePath(file.path('www',
                                      paste("/RastImg/NewS/",fName(),sep='')))
      list(src=path,
           alt=fName())
    }
  }else{ # Calls elevation images. 
    if(input$PrePost == T){
      path <- normalizePath(file.path('www',
                                      paste("/RastImg/OldE/",fName(),sep='')))
      list(src=path,
           alt=fName())
      }
  else{
    path <- normalizePath(file.path('www',
                                    paste("/RastImg/NewE/",fName(),sep='')))
    list(src=path,
         alt=fName())
  ...
  test<- eventReactive(input$MapMine_shape_click,{
    name1<- id()$id
    if (id()$group == 'Watersheds'){name2<- as.numeric(name1)
                                       return(slope[which(slope$HUC12== name2),])}
    else{return(slope[which(slope$NAME_1== name1),])}
  })
  ...
  entireWatershed<- reactive({
    ...
    s.dens.plot <- s.dens + geom_density(alpha=0.8) + #ggtitle('c)') 
      theme(legend.position=c(-.35,.7),legend.title=element_blank()) + 
      scale_fill_manual(values=cols, name='') + ylim(0,0.07)+
      xlab('Slope (degrees)') + ylab('') + xlim(0,50) + 
      ggtitle(bquote(atop(paste(bold('Over Whole Watershed Area')),
  ...
  minedOnly<- reactive({
    ...
    d <- round(vals[2]-vals[1],1)
    title <- bquote(atop(paste(bold('Over Mined Areas')),
              paste('slope changed by ',.(d),degree)))
    
    s.dens <- ggplot(test()[which(test()$mining=='Mined'),],aes(value.deg,fill=variable))
    s.dens.plot <- s.dens + geom_density(alpha=0.8) + #ggtitle('c)') 
      ...
      scale_fill_manual(values=cols,
                        breaks=c("OldSlope", "LidarSlope"),
                        labels=c("Pre-Mining", "Post-Mining"),guide=F)
    return(s.dens.plot)
  }) 
  #Call plots
  ...
  }else{    #Closes slope if statement
    ...
    #Ggplot to compare changes in Elevation---------####-------
      ...
      test<- eventReactive(input$MapMine_shape_click,{
        ...
        else{return(elev[which(elev$NAME_1== name1),])}
      })
      ...
      entireWatershed<- reactive({
        ...
        e.dens.plot <- e.dens + geom_density(alpha=0.8) + #ggtitle('c)')
          theme(legend.position=c(-.35,.8),legend.title=element_blank())+
      ...
      minedOnly<- reactive({
        ...
        d <- round(vals[2]-vals[1],1)
        e.dens <- ggplot(test()[which(test()$mining=='Mined'),],aes(value,fill=variable))
        e.dens.plot <- e.dens + geom_density(alpha=0.8) + #ggtitle('c)') 
          scale_fill_manual(values=cols,guide=FALSE)  +
          xlim(xlims[1],xlims[2]) + 
          xlab('Elevation (meters)') + ylab('Density') + 
          ggtitle(bquote(atop(paste(bold('Over Mined Areas')),
                               paste('Elevation changed by ',.(d),'m'))))
        return(e.dens.plot)
        })
      output$gplots<- renderPlot({
